  ...
  if (token) {
    ...
    .then(([DS, Productos,Usuario]) => {
      const tipoUsuario = Usuario.length > 0 ? Usuario[0].type : null;
        res.render('DS-38', { DS: DS, productos: Productos,tipoUsuario: tipoUsuario});
    })
...
module.exports.Crear = async (req, res) => {
  ...
  try {
    ...
    if (producto.Tipo == "Bar") {
      const ds = await DiasSol.findOne({ DS: "DS-38", Ingreso: Fecha });
      if (!ds) {
        return res.status(404).send("No se encontró el día de sol DS-38 para hoy");
      }
