const DS = require('../models/DDS-48')
  ...
  if (token) {
    ...
    .then(([DS, Productos,Usuario]) => {
      const tipoUsuario = Usuario.length > 0 ? Usuario[0].type : null;
        res.render('DDS-48', { DS: DS, productos: Productos,tipoUsuario: tipoUsuario});
    })
...
module.exports.Crear = async (req, res) => {
  ...
  try {
    ...
    if (producto.Tipo == "Bar") {
      const ds = await DiasSol.findOne({ DS: "DDS-48", Ingreso: Fecha });
      if (!ds) {
        return res.status(404).send("No se encontró el día de sol DDS-48 para hoy");
      }
